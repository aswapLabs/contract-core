script {
    ...
    fun main(account: signer) {
        let token2 = Account::withdraw<Token2>(&account, 100000000);

        let (reserve_1, reserve_2 , _) = Pair::get_reserves<Token1, Token2>();
        Debug::print(&reserve_1);

        let amount_out_with_fee = 100000000 * 997;
        let denominator = reserve_2 * 1000 + amount_out_with_fee;
        let r = SafeMath::safe_mul_div(amount_out_with_fee, reserve_1, denominator);
