...
module Token1 {
    ...
    public fun init(account: signer) {
        Token::register_token<Token1>(&account, PRECISION);
        Account::do_accept_token<Token1>(&account);
    ...
    public(script) fun get_balance(account: signer): u128 {
        let num = Account::balance<Token1>(Signer::address_of(&account));
        Debug::print(&num);
        num
    }

    public fun get_balance_cli(account: &signer): u128 {
        Account::balance<Token1>(Signer::address_of(account))
    }

    fun mint(account: &signer, amount: u128) {
        let token = Token::mint<Token1>(account, amount);
        Account::deposit_to_self<Token1>(account, token);
    }
    ...
    public fun is_token1<TokenType: store>(): bool {
        Token::is_same_token<Token1, TokenType>()
    }
